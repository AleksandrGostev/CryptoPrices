...
import {
  getAllCoinsBasicData,
  getCoinDataById,
  getCoinExternalIdByCoinId,
  getCoinsWithOutdatedData,
  upsertCoinMarketData
} from './database';
...
coinsRouter.get('/:coinId/tickers', cacheMiddleware, async (req, res) => {
  ...
  const tickersResponse = await axios.get(
    `https://api.coingecko.com/api/v3/coins/${coinData.external_id}/tickers`,
    {
      params: {
  ...
  const data = tickersResponse.data.tickers;

  const chartDataResponse = await axios.get(
    `https://api.coingecko.com/api/v3/coins/${coinData.external_id}/market_chart`,
    {
      params: {
        vs_currency: 'usd',
        days: 30
  ...
  const prices = chartDataResponse.data.prices.map(([time, price]) => ({
