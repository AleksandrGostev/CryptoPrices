...
export const getCoinsWithOutdatedData = async () => {
  return db('coin as c')
    ...
    .whereRaw(
      `cmd.updated_at is null OR (EXTRACT(EPOCH FROM (? - cmd.updated_at)) / 60 > 30)`,
      [db.fn.now()]
    );
};
...
export const getCompareToExternalIds = async (coinId: string) => {
  return db('coin_external_provider')
    .whereNotNull('coin_id')
    .andWhereNot('coin_id', coinId)
    .pluck('external_id');
};

export const getCoinExternalIdByCoinId = async (coinId: string) =>
  db('coin_external_provider').select('external_id').where('coin_id', coinId);
